...
var pacientes = mysqlTable("pacientes", {
...
var profissionais = mysqlTable("profissionais", {
  id: varchar("id", { length: 64 }).primaryKey(),
  nome: varchar("nome", { length: 255 }).notNull(),
  whatsapp: varchar("whatsapp", { length: 20 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
...
var especialidades = mysqlTable("especialidades", {
  id: varchar("id", { length: 64 }).primaryKey(),
  nome: varchar("nome", { length: 255 }).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var agendamentos = mysqlTable("agendamentos", {
  id: varchar("id", { length: 64 }).primaryKey(),
  ...
  pacienteId: varchar("pacienteId", { length: 64 }).notNull(),
  profissional: varchar("profissional", { length: 255 }).notNull(),
  profissionalId: varchar("profissionalId", { length: 64 }).notNull(),
  especialidade: varchar("especialidade", { length: 255 }).notNull(),
  especialidadeId: varchar("especialidadeId", { length: 64 }).notNull(),
...
async function getAllProfissionais() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(profissionais);
}
async function createProfissional(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(profissionais).values(data);
}
...
async function deleteEspecialidade(id) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(especialidades).where(eq(especialidades.id, id));
}
...
var appRouter = router({
  ...
  profissionais: router({
    list: publicProcedure.query(async () => {
      return await getAllProfissionais();
    }),
    create: publicProcedure.input(
      z2.object({
        id: z2.string(),
      ...
      await createProfissional(input);
      return { success: true };
    }),
    delete: publicProcedure.input(z2.object({ id: z2.string() })).mutation(async ({ input }) => {
      await deleteProfissional(input.id);
      return { success: true };
    })
  }),
  // Especialidades
  especialidades: router({
    ...
    create: publicProcedure.input(
      ...
      await createEspecialidade(input);
      return { success: true };
    }),
    delete: publicProcedure.input(z2.object({ id: z2.string() })).mutation(async ({ input }) => {
      await deleteEspecialidade(input.id);
      return { success: true };
    })
  }),
  // Agendamentos
  agendamentos: router({
    list: publicProcedure.query(async () => {
      return await getAllAgendamentos();
    }),
    create: publicProcedure.input(
      z2.object({
        id: z2.string(),
        data: z2.string(),
        horaInicio: z2.string(),
        horaFim: z2.string(),
        paciente: z2.string(),
        pacienteId: z2.string(),
        profissional: z2.string(),
        profissionalId: z2.string(),
        especialidade: z2.string(),
        especialidadeId: z2.string(),
        sala: z2.string(),
        telefone: z2.string().optional(),
